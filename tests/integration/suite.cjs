...
exports.run = async function () {
  ...
  try {
    ...
    await workbench.update('colorCustomizations', { 'editor.lineHighlightBorder': '#abcdef' }, vscode.ConfigurationTarget.Global);
    await api.testing.homeMessage({ action: 'personalize', displayName: 'PRIVATE_GREETING_NEVER_SEND', experience: 'concise', accent: 'ocean' });
    assert.equal(vscode.workspace.getConfiguration('caled').get('studio.personalization').accent, 'ocean');
    const colors = vscode.workspace.getConfiguration('workbench').get('colorCustomizations');
    assert.equal(colors['editor.lineHighlightBorder'], '#abcdef');
    assert.equal(colors['[Caled Dark]']['button.background'], '#9bc4d8');
    checks.push('personal accent applied without replacing unrelated editor preferences');
    const brief = { goal: 'BRIEF_SUM_GOAL: explicar y corregir la suma', audience: 'Aprendices', criteria: ['sum(2, 3) devuelve 5'], kind: 'learn' };
    await api.testing.homeMessage({ action: 'saveBrief', brief });
    assert.equal(api.testing.brief().goal, brief.goal);
    await assert.rejects(api.testing.homeMessage({ action: 'saveBrief', brief: { ...brief, goal: 'x'.repeat(2001) } }));
    assert.equal(api.testing.brief().goal, brief.goal);
    await api.testing.homeMessage({ action: 'journey', step: 'plan', brief });
    await waitFor(() => api.testing.events().some(event => event.type === 'draft'), 'notebook instruction draft');
    assert.equal(vscode.workspace.getConfiguration('caled').get('agent.profile'), 'guide');
    assert.equal(requests.length, 0, 'Preparing a notebook instruction must never call the model');
    assert.ok(api.testing.events().some(event => event.type === 'draft' && event.text.includes(brief.goal)));
    await config.update('agent.profile', 'builder', vscode.ConfigurationTarget.Global);
    checks.push('notebook saved, invalid replacement rejected, plan prepared without sending');
    ...
    assert.ok(requests[0].messages.at(-1).content.includes('BRIEF_SUM_GOAL'));
    assert.ok(!JSON.stringify(requests).includes('PRIVATE_GREETING_NEVER_SEND'));
    assert.ok(requests[0].messages[0].content.includes('concisa'));
    checks.push('project goal and explanation level reach AI while greeting name stays local');
    ...
    const secondary = vscode.Uri.file(path.join(root, '..', 'secondary'));
    await fs.mkdir(secondary.fsPath, { recursive: true });
    const foldersChanged = () => new Promise((resolve, reject) => {
      const timer = setTimeout(() => { subscription.dispose(); reject(new Error('Workspace event did not arrive')); }, 5000);
      const subscription = vscode.workspace.onDidChangeWorkspaceFolders(() => { clearTimeout(timer); subscription.dispose(); resolve(); });
    });
    let changed = foldersChanged();
    assert.equal(vscode.workspace.updateWorkspaceFolders(1, 0, { uri: secondary }), true); await changed;
    await vscode.commands.executeCommand('caled.reindex');
    assert.ok(api.testing.stats().indexedFiles >= 1, 'The index must restart after a workspace-folder change');
    changed = foldersChanged();
    assert.equal(vscode.workspace.updateWorkspaceFolders(1, 1), true); await changed;
    await vscode.commands.executeCommand('caled.reindex');
    assert.ok(api.testing.stats().indexedFiles >= 1);
    checks.push('workspace-folder changes recreate the index worker and clear stale drafts');
    if (process.env.CALED_TEST_CAPTURE) {
      ...
      await api.testing.appearance();
      await api.testing.homeMessage({ action: 'personalize', displayName: '', experience: 'guided', accent: process.env.CALED_TEST_APPEARANCE === 'light' ? 'clay' : 'ocean' });
      await api.testing.homeMessage({ action: 'saveBrief', brief: { goal: 'Una página para mi cafetería, con el menú y una forma sencilla de encontrarnos.', audience: 'Personas que viven o trabajan cerca.', criteria: ['El menú se puede leer desde un teléfono.', 'El horario y la dirección están a la vista.', 'La página funciona con teclado.'], kind: 'web' } });
